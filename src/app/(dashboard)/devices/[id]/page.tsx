...
async function getTelemetryHistory(deviceId: string) {
    try {
        const res = await fetchWithAuth(`/api/admin/devices/${deviceId}/telemetry?limit=10000`, { cache: "no-store" });
        if (!res.ok) {
...
export default async function DeviceDetailPage({ params }: PageProps) {
    ...
    try {
        device = await getDeviceDetails(id);
        if (device) {
            telemetryHistory = await getTelemetryHistory(id);
        }
    } catch (e: any) {
